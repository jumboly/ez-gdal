...
- **対象 OS**:
  ...
  - Portable single-file: Windows x64 / Linux x64 / macOS arm64
...
> **Windows プラグイン作者向け SDK**: `Jumboly.EzGdal.win-x64` nupkg は `sdk/` フォルダに `gdal.lib` (import library) + ヘッダ + CMake config を同梱しています。Windows プラグインのビルド手順は [`docs/plugin-authoring.md` §4.1](docs/plugin-authoring.md#windows) を参照。
