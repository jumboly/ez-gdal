...
- **対象 OS**: Windows x64 / Linux x64 / macOS x64+arm64
- **GDAL バージョン**: 3.12.x（MaxRev.Gdal NuGet 経由）
...
ezgdal 自体は MIT。同梱する GDAL / PROJ / GEOS / SQLite ほかライブラリのライセンスは `licenses/` を参照（配布物に同梱）。
