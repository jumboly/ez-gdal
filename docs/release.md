...
- 5 RID 分 (osx-arm64 / osx-x64 / linux-x64 / linux-arm64 / win-x64) のクロスプラットフォーム検証は実機 CI runner でしか担保できない (ローカルはクロスビルドのみ)
...
5 つの PackageId が NuGet.org 上に予約される:
...
すべて NuGet.org 上限 250MB に収まる。合計 push 量 ~245MB。展開サイズは各 RID で ~150MB (`~/.dotnet/tools/.store/jumboly.ezgdal.<rid>/` 配下)。
